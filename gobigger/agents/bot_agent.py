...
class BotAgent(BaseAgent):
    ...
    def transform_obs_to_overlap(self, obs):
        overlap = {"food": [], "thorns": [], "spore": [], "clone": []}
        for item in obs:
            if item.ball_type == 1:
                overlap['food'].append([item.position_x, item.position_y, item.radius])
            if item.ball_type == 3:
                overlap['spore'].append([item.position_x, item.position_y, item.radius])
            if item.ball_type == 2:
                overlap['thorns'].append([item.position_x, item.position_y, item.radius])
            if item.ball_type == 4:
                overlap['clone'].append([item.position_x, item.position_y, item.radius,
                                         item.owner, item.team_name])
        return overlap
